...
For me, the framework that seems to answer most to my questions, is that of Langacker's **Cognitive Grammar** (1987; 1991 etc.):
* dynamic
* usage-based
...
vqilaifreqD <- read_csv("../data/V-QILAI-freq.csv") %>%
  select(hit, sum)
...
vqilaiframe <- read_csv("../data/V-QILAI-freq-frame.csv")
...
>The dynamic conception of a path is therefore immanent in the conception of actual motion." (Langacker 2008:529)
...
tingqilai <- read_csv("../data/tingqilai.csv") 
...
* 'radical support' for literary ideophones (Van Hoey *in press*), e.g. `雨 in 霏霏`
...
moqilai <- read_csv("../data/moqilai.csv") 
...
moqilai_ideos <- read_csv("../data/moqilai_ideos.csv") 
...
* This helps the formation of different schemas (based on real language data) that will help apprehend next encounters with of the same construction (and different levels of abstraction).
...
* Particularly in connection to my own studies, this network of .sc[perception verb + qilai] constructions can help me discover new ideophones -- probably relatable to evidentiality (cf. Vygotsky 1983; San Roque et al. 2015; Van Hoey & Lu *in prep.*). However, this deserves further research.
...
* I am not entirely in agreement with this list. There seem many 'mistakes'?
