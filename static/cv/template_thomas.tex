...
\subsectionfont{\rmfamily\mdseries\large\slshape}
...
% Use fontawesome. Note: you'll need TeXLive 2015. Update. and also academicons
$if(fontawesome)$\usepackage{fontawesome5}$endif$
$if(academicons)$\usepackage{academicons}$endif$
...
$if(affiliation)$\moveleft.5\hoffset\centerline{$affiliation$}$endif$
% first column
\begin{minipage}[t]{0.7\textwidth}
$if(email)$$if(fontawesome)$\faEnvelope \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{1 mm}$endif$\\
$if(email2)$$if(fontawesome)$\faEnvelope \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email2$} \hspace{1 mm}$endif$\\
$if(phone)$$if(fontawesome)$ \faPhone \hspace{1 mm}$else$\emph{Phone:}$endif$  $phone$  \hspace{1 mm} $endif$\\
$if(addressuni)$$if(fontawesome)${\large \faUniversity} \hspace{1 mm}$else$\emph{Address:}$endif${$addressuni$}$endif$\\
$if(address)$$if(fontawesome)${\large \faHome} \hspace{1 mm}$else$\emph{Address:}$endif${$address$}$endif$\\
...
$if(orcid)$$if(academicons)$\aiOrcidSquare \hspace{1 mm}$else$\emph{Orcid:}$endif$ \href{http://orcid.org/$orcid$}{\tt $orcid$} \hspace{1 mm} $endif$
\end{minipage}
