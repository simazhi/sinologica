...
```{r zhuozhuocompute, warning=FALSE, cache=TRUE, echo=FALSE, message=FALSE, fig.cap="Nearest neighbours of zhuozhuo 灼灼", fig.align='center'}
...
plot <- db %>%
  ...
  filter(! collocate == "」") %>%
  mutate(highlight = 
           case_when(str_detect(collocate, "[桃金光火珠珊瑚亮姿黃菊華紅綠蓁雲鬼]") ~ "in original", 
  ...
  theme(legend.title = element_blank(),
        ...
        plot.title = element_text(family = "Noto Sans CJK TC", hjust=-0)) +
  ggtitle("Distributional semantic nearest neighbours of zhuozhuo 灼灼") +
...
Step 3: frequencies and co-occurrence strength. Next, the strength of the co-occurrence of the skip-grams that resulted from the previous function needs to be weighted. For this we will also calculate the PMI value for each pair of words. This is then stored in a tidy data frame44 (Wickham 2014). This function is called tidy_pmi() and results in an extremely long and thin table.
Step 4: similarity. To reduce the dimensionality the widely_svd() function from the widyr package is used. In her example, Silge uses 256 dimensions, which seems to be a good number for our data as well. This eventually results in a data frame object tidy_word_vectors, which can be queried for the nearest neighbours of a target word.
...
conclusion: good method and supplement

but needs manual tweaking
...
auditory
ideophones, but first computation and then finegrained 
...
a. Is there any variation at play, and if so, in what ways?
b. How have these structure evolved over time?
c. What constructions do ideophones appear in and what are their functions?
